package cr.ac.una.controlfinancierocameraAndres.service
...
import cr.ac.una.controlfinancierocameraAndres.R
...
class LocationService : Service() {
    ...
    private fun requestLocationUpdates() {
        val locationRequest = LocationRequest.Builder(
            Priority.PRIORITY_HIGH_ACCURACY, 0L// El intervalo de tiempo puede ser 0 para depender solo de la distancia
        ).apply {
            setMinUpdateDistanceMeters(15.0f) // Cambia este valor según el umbral de distancia que desees
        }.build()
            ...
            placeResponse.addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    topPlaces.forEach { placeLikelihood ->
                        ...
                        //mandar placeLikelihood.place.name para buscarlo en la wiki
                        var texto = placeLikelihood.place.name?.replace(" ", "_")
                        textoBusqueda = texto.toString()
                    }
                } else {
